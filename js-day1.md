...
The browser will download `foo.js` first, and execute all the contents line by line, then it will download `bar.js`, execute that, etc... 

Also, it's often the case that you'll want to use something that `foo.js` provides inside `bar.js`.  To do this you must ensure that `foo.js` is loaded first.  
